...
export class SuccessfullPaymentPlanPage {
  ...
  ngOnInit() {
    ...
    const stripeAccountCreated = this.localStorageService.getItem(
      `stripeAccountCreated-${companyId}`
    );
  ...
  createStripeAccount(companyId: number) {
    this.companyService
      .getByField<Company>('companies', 'id', companyId)
      .then((company) => {
        if (!company?.email) {
        ...
        this.stripeService
          .createConnectedAccount(company.email)
  ...
  createAccountLink(accountId: string, email: string) {
    this.stripeService
      ...
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (accountLink) => (window.location.href = accountLink.url),
      });
  }
